...
trait MediaStreamErrorEvent extends Event {
  val error: MediaStreamError | Null
}
...
object MediaStreamErrorEvent {
  ...
  def apply(
    ...
    __obj.asInstanceOf[MediaStreamErrorEvent]
  }
  @scala.inline
  implicit class MediaStreamErrorEventOps[Self <: MediaStreamErrorEvent] (val x: Self) extends AnyVal {
    @scala.inline
